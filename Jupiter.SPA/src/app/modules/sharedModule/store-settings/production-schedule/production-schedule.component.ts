...
export class ProductionScheduleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.changeProductionScheduleExpectedDateBoxHidden(true);
  ...
  getProductionSchedule() {
    this.storebuilderService.getTeamStore(this.storeId).subscribe(
      (response) => {
        this.setProductionScheduleValues(response.data.productionSchedule)
      },
      (error) => {
        console.log(error)
  ...
  setProductionScheduleValues(productionSchedule){
    let dateArr = productionSchedule.expectedDate.split("/") //dd/mm/yyyy
    ...
    this.storeService.updateProductionSchedule(this.storeId, {expectedDate,productionScheduleType}).subscribe(
      (response) => {
        if(response.status == 1){
          ...
          this.router.navigateByUrl("/storebuilder/storedetails/storesettings/shippingmethods");
