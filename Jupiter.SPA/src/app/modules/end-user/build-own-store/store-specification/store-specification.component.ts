...
export class StoreSpecificationComponent implements OnInit {
  ...
  productSelection(event) {
    ...
    // if (product && this.productList.length > 0 && product.name != null && product.name != undefined) {
  ...
  getProduct(categoryId: any) {
    this.productService.getProduct({ viewType: 2, categoryId }).subscribe(
      (response) => {
  ...
  calculateDecoPricing(garment, artMapping) {
    var totalDecoPricing = 0;
    for (var i = 0; i < artMapping.controls.length; i++) {
  ...
  personalizationTypeSelected(index, personalize, j, garment, personalization) {
    ...
    var cost = this.personalizationTypes.find(x => x.id == personalize.controls.personalizedTypeId.value).cost;
    personalize.patchValue({ personalizedCost: cost });

    this.totalPersonalizationCost = 0;
    for (var i = 0; i < personalization.controls.length; i++) {
  ...
  createStore() {
    var garmentArray = this.storeRequestForm.value;
  ...
  openAddColorModal(artMap, j, garment, artMapping, index) {
    ...
    if (artMap.get('colors').value != null) {
      var result = this.colorArray.filter(
        function (e) {
  ...
  updateArtColor() {
    var colorCount = this.selectedArtMap.get('noOfColors').value;
